	  ...
	  # open it up, the a means we will append to it
		with open(outfile_path, 'a') as csvfile:
			writer = csv.writer(csvfile, delimiter='|')
	    		...
	    		writer.writerow(headers)
			url = requests.get("https://graph.facebook.com/v2.6/%s?fields=name,feed.limit(15){from,created_time,comments{from,created_time,comments,message,story,permalink_url,full_picture,story_tags,link,description,type},message,story,permalink_url,full_picture,story_tags,link,description,type}&access_token=%s&date_format=Y-m-d H:i:s" % (object_id,access_token))
			...
			for item in parsed_json['feed']['data']:
				...
				try:
					for i in range(len(item['comments']['data'])):
						for j in range(len(item['comments']['data'][i]['comments']['data'])):
							row = []
							row.append(object_id)
							row.append('subcomment')
							row.append(parsed_json['name'])
		                                        row.append(item['comments']['data'][i]['comments']['data'][j]['id'])
                                                	row.append(item['permalink_url'])
                                                	row.append(unicode(item['comments']['data'][i]['comments']['data'][j]['from']['name']).encode('utf-8'))
                                                	row.append('http://www.facebook.com/' + item['comments']['data'][i]['comments']['data'][j]['from']['id'])
                                                	row.append(item['comments']['data'][i]['comments']['data'][j]['from']['id'])
		                                        row.append('http://graph.facebook.com/' + item['comments']['data'][i]['comments']['data'][j]['from']['id'] + '/picture')
                                                	row.append(item['comments']['data'][i]['comments']['data'][j]['created_time'])
                                                	row.append((unicode(item['comments']['data'][i]['comments']['data'][j]['message']).encode('utf-8')).replace('|',' '))
  			                                row.append('')
                                                	row.append('')
                                                	if item.has_key("full_picture"):
                                                        	row.append(item["full_picture"])
